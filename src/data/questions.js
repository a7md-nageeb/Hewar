export const questions = [
    // --- Funny ---
    {
        id: 'f1',
        text_en: "If I were a fruit, what would I be and why?",
        text_ar: "لو كنت فاكهة، تفتكر هكون إيه وليه؟",
        category: ['Lovers', 'Friends', 'Family', 'First Date', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f2',
        text_en: "What’s the weirdest nickname you can come up with for me?",
        text_ar: "إيه أغرب اسم دلع ممكن تطلعه عليا؟",
        category: ['Friends', 'Family', 'Lovers', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f3',
        text_en: "Who in this group would survive a zombie apocalypse the longest—and who dies first?",
        text_ar: "مين فينا هيعيش أطول وقت لو حصل هجوم زومبي - ومين هيموت أول واحد؟",
        category: ['Friends', 'Family', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f4',
        text_en: "What’s something I would totally go viral for?",
        text_ar: "إيه الحاجة اللي ممكن تخليني تريند؟",
        category: ['Friends', 'Lovers', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f5',
        text_en: "If I had a theme song play every time I walked into a room, what would it be?",
        text_ar: "لو ليا أغنية بتشتغل كل ما أدخل أي مكان، هتكون إيه؟",
        category: ['Friends', 'Coworkers', 'Lovers', 'First Time Meeting'],
        genre: 'Funny'
    },
    // Who is more (Funny)
    {
        id: 'f6',
        text_en: "Who is more likely to become friends with a random stranger they meet in public?",
        text_ar: "مين فينا ممكن يصاحب أي حد يقابله في الشارع عادي؟",
        category: ['Friends', 'Lovers', 'Family', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f7',
        text_en: "Who is more likely to laugh at a joke that nobody else finds funny?",
        text_ar: "مين فينا بيضحك على نكت محدش بيضحك عليها غيره؟",
        category: ['Friends', 'Lovers', 'Family', 'Coworkers'],
        genre: 'Funny'
    },
    {
        id: 'f8',
        text_en: "Who is more likely to get distracted by something shiny or colorful?",
        text_ar: "مين فينا بيتسحل مع أي حاجة بتلمع أو ملونة؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f9',
        text_en: "Who's more likely to be caught snacking late at night?",
        text_ar: "مين فينا بيحب ينقنق في الأكل بليل؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f10',
        text_en: "Who's more likely to hide behind the other person when they see a bug?",
        text_ar: "مين فينا هيستخبى ورا التاني لما يشوف صرصار؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f11',
        text_en: "Who's more likely to stay up all night scrolling on their phone?",
        text_ar: "مين فينا بيسهر طول الليل يقلب في الموبايل؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f12',
        text_en: "Who's more likely to say, \"I'm not upset,\" but is actually upset?",
        text_ar: "مين فينا بيقول \"أنا مش زعلان\" وهو بيموت من الزعل؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },
    {
        id: 'f13',
        text_en: "If I lost my keys, where's the first place you'd look?",
        text_ar: "لو ضيعت مفاتيحي، أول مكان هتدور فيه فين؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f14',
        text_en: "If zombies took over, what would you want me to do?",
        text_ar: "لو الزومبي احتلوا العالم، عايزني أعمل إيه؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },
    {
        id: 'f15',
        text_en: "If I gave you a makeover, what would you be most scared of me changing?",
        text_ar: "لو قررت أغيرلك ستايلك، إيه أكتر حاجة هتخاف أغيرها؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f16',
        text_en: "If I got banned from a store, which one would it be and why?",
        text_ar: "لو اتمنعت أدخل محل معين، تفتكر هيكون إيه وليه؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f17',
        text_en: "If we were stuck in a time loop, repeating the same day, how would we make it fun?",
        text_ar: "لو اليوم ده بيتعاد كل يوم، هنعمل إيه عشان ننبسط؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f18',
        text_en: "If someone gave you a million dollars just to embarrass me in public, what would you do?",
        text_ar: "لو حد عرض عليك مليون دولار عشان تكسفني قدام الناس، هتعمل إيه؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },
    {
        id: 'f19',
        text_en: "If you got a call saying I'd been detained, what's your guess for why?",
        text_ar: "لو جالك تليفون إن اتقبض عليا، تفتكر هتكون التهمة إيه؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f20',
        text_en: "If you had to guess what I talked about the most as a kid, what would it be?",
        text_ar: "تفتكر وأنا صغير كنت برغي في إيه أكتر حاجة؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f21',
        text_en: "If I suddenly had a completely different personality, would you still love me?",
        text_ar: "لو شخصيتي اتغيرت تماماً فجأة، هتفضل تحبني؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f22',
        text_en: "If my mom calls asking where I am and I'm ignoring her, what would you say I'm doing?",
        text_ar: "لو ماما كلمتك تسأل عليا وأنا مطنشها، هتقولها أنا فين؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },
    {
        id: 'f23',
        text_en: "What do you know about me that would ruin my chances of ever becoming president?",
        text_ar: "إيه اللي تعرفه عني وممكن يخليني أعمر ما أبقى رئيس جمهورية؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f24',
        text_en: "What's the creepiest thing I've said to you?",
        text_ar: "إيه أكتر حاجة مريبة قلتها ليك قبل كده؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },
    {
        id: 'f25',
        text_en: "If you could instantly make me an expert at one thing, what would it be?",
        text_ar: "لو بإيدك تخليني خبير في حاجة واحدة دلوقتي، هتختار إيه؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f26',
        text_en: "If you had to pick one animal for us to become, what would we be?",
        text_ar: "لو هنقلب حيوانات، تفتكر هنكون إيه؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f27',
        text_en: "Do you think you could survive in a reality TV show?",
        text_ar: "تفتكر ممكن تستحمل تعيش في برنامج واقعي؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'Funny'
    },
    {
        id: 'f28',
        text_en: "If you had to be conjoined twins with someone from our friend group, who would it be and why?",
        text_ar: "لو لازم تلزق في حد من صحابنا وتعيشوا توأم ملتصق، هتختار مين؟",
        category: ['Friends', 'Lovers'],
        genre: 'Funny'
    },

    // --- Deep ---
    {
        id: 'd1',
        text_en: "What’s something you wish more people understood about you?",
        text_ar: "إيه الحاجة اللي نفسك الناس تفهمها عنك أكتر؟",
        category: ['Lovers', 'Friends', 'Family', 'First Date'],
        genre: 'Deep'
    },
    {
        id: 'd2',
        text_en: "When do you feel most like yourself?",
        text_ar: "إمتى بتحس إنك على طبيعتك بجد؟",
        category: ['Lovers', 'Friends', 'First Date'],
        genre: 'Deep'
    },
    {
        id: 'd3',
        text_en: "What’s one memory that shaped who you are today?",
        text_ar: "إيه الذكرى اللي أثرت في شخصيتك النهاردة؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Deep'
    },
    {
        id: 'd4',
        text_en: "What’s something you’re currently struggling with but rarely talk about?",
        text_ar: "إيه الحاجة اللي تاعباك الفترة دي ومش بتتكلم عنها؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd5',
        text_en: "What’s a belief or value that you’ll never compromise on?",
        text_ar: "إيه المبدأ اللي عمرك ما تتنازل عنه؟",
        category: ['Lovers', 'Friends', 'Family', 'Coworkers'],
        genre: 'Deep'
    },
    // Boyfriend/Girlfriend Deep Questions
    {
        id: 'd6',
        text_en: "The moment I knew I was falling for you was…",
        text_ar: "اللحظة اللي عرفت فيها إني بحبك كانت...",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd7',
        text_en: "I hope you never stop…",
        text_ar: "أتمنى عمرك ما تبطل...",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd8',
        text_en: "My favorite memory of us still is…",
        text_ar: "أحلى ذكرى لينا مع بعض لحد دلوقتي هي...",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd9',
        text_en: "My favorite part of your personality is…",
        text_ar: "أكتر حاجة بحبها في شخصيتك هي...",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd10',
        text_en: "Being loved by you feels like…",
        text_ar: "حبك بيحسسني إن...",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd11',
        text_en: "What do you know about me now that you wish you knew the first day we met?",
        text_ar: "إيه اللي عرفته عني دلوقتي وكنت تتمنى تعرفه من أول يوم؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd12',
        text_en: "If you had to describe the way I love in one sentence, what would it be?",
        text_ar: "لو هتوصف حبي ليك في جملة، هتقول إيه؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd13',
        text_en: "What feels more intense, falling in love or falling out of love?",
        text_ar: "تفتكر إيه إحساسه أقوى، الوقوع في الحب ولا إن الحب ده يخلص؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd14',
        text_en: "Is it harder to forgive ourselves or to forgive those who hurt us?",
        text_ar: "تفتكر أصعب نسامح نفسنا ولا نسامح اللي وجعنا؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd15',
        text_en: "What type of dad do you think I'll be?",
        text_ar: "تفتكر هكون أب عامل إزاي؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd16',
        text_en: "What was your favorite day with me?",
        text_ar: "إيه أحلى يوم قضيته معايا؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd17',
        text_en: "What do you hope our future home looks like?",
        text_ar: "نفسك بيتنا في المستقبل يكون شكله إزاي؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd18',
        text_en: "What's something I do that worries you, because you care about me?",
        text_ar: "إيه الحاجة اللي بعملها وبتقلقك عليا؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Deep'
    },
    {
        id: 'd19',
        text_en: "When did you realize I loved you—like, actually not me just saying it?",
        text_ar: "إمتى اتأكدت إني بحبك بجد، مش مجرد كلام؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd20',
        text_en: "What's the most supportive thing I've done for you that you'll never forget?",
        text_ar: "إيه أكتر موقف وقفت فيه جنبك وعمرك ما هتنساه؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Deep'
    },
    {
        id: 'd21',
        text_en: "If you could grant us one wish for our future together, what would it be?",
        text_ar: "لو ليك أمنية واحدة لمستقبلنا سوا، هتكون إيه؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd22',
        text_en: "What changed in you after we met?",
        text_ar: "إيه اللي اتغير فيك بعد ما عرفتني؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd23',
        text_en: "Who was the first person you told about me?",
        text_ar: "مين أول حد حكيتله عني؟",
        category: ['Lovers'],
        genre: 'Deep'
    },
    {
        id: 'd24',
        text_en: "What part of your personality gets stronger when you're with me?",
        text_ar: "إيه الجزء اللي في شخصيتك بيقوى لما بتكون معايا؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },
    {
        id: 'd25',
        text_en: "What's the one question you've always wanted to ask me but never did?",
        text_ar: "إيه السؤال اللي كان نفسك تسألهولي بس متردد؟",
        category: ['Lovers', 'Friends'],
        genre: 'Deep'
    },

    // --- Awkward ---
    {
        id: 'u1',
        text_en: "What’s something about me that annoys you but you’ve never said out loud?",
        text_ar: "إيه الحاجة اللي بتضايقك فيا بس عمرك ما قلتها؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Awkward'
    },
    {
        id: 'u2',
        text_en: "What’s the most embarrassing thing you’ve done that I don’t know about?",
        text_ar: "إيه أكتر حاجة محرجة عملتها وأنا معرفش عنها حاجة؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u3',
        text_en: "Have you ever pretended to agree with me just to avoid conflict?",
        text_ar: "عمرك عملت نفسك موافقني في الرأي بس عشان تريح دماغك؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u4',
        text_en: "If you could change one thing about me, what would it be?",
        text_ar: "لو بإيدك تغير فيا صفة واحدة، هتكون إيه؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u5',
        text_en: "What’s something you’ve lied about recently?",
        text_ar: "إيه الحاجة اللي كدبت فيها قريب؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u6',
        text_en: "When's the last time you ghosted someone and why?",
        text_ar: "إمتى آخر مرة عملت \"Ghosting\" لحد وليه؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u7',
        text_en: "What is the easiest way to break your trust?",
        text_ar: "إيه أسرع طريقة تخسر بيها ثقتك؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },
    {
        id: 'u8',
        text_en: "What's a common trait shared by everyone else that you've dated?",
        text_ar: "إيه الصفة المشتركة بين كل اللي ارتبطت بيهم قبلي؟",
        category: ['Lovers', 'Friends'],
        genre: 'Awkward'
    },

    // --- What If ---
    {
        id: 'w1',
        text_en: "What if we swapped lives for a week — what would shock you the most?",
        text_ar: "لو بدلنا حياتنا مع بعض لمدة أسبوع، إيه أكتر حاجة هتصدمك؟",
        category: ['Lovers', 'Friends', 'Family', 'First Date'],
        genre: 'What If'
    },
    {
        id: 'w2',
        text_en: "What if you had to describe me in 3 emojis only — which ones would you choose?",
        text_ar: "لو هتوصفني بـ ٣ إيموجي بس، هتختار إيه؟",
        category: ['Friends', 'Coworkers', 'First Time Meeting', 'Lovers'],
        genre: 'What If'
    },
    {
        id: 'w3',
        text_en: "What if we were in a movie — what roles would we each play?",
        text_ar: "لو احنا في فيلم، كل واحد فينا هيلعب دور إيه؟",
        category: ['Friends', 'Family', 'Coworkers', 'First Time Meeting'],
        genre: 'What If'
    },
    {
        id: 'w4',
        text_en: "What if you had to relive one day of your life forever — which would you pick?",
        text_ar: "لو لازم تعيش يوم واحد من حياتك وتعيده للأبد، هتختار أنهي يوم؟",
        category: ['Friends', 'Lovers', 'Family'],
        genre: 'What If'
    },
    {
        id: 'w5',
        text_en: "What if you had to pitch our friendship/relationship like a product — how would you describe it?",
        text_ar: "لو هتسوق لعلاقتنا كأنها منتج، هتقول عنها إيه؟",
        category: ['Friends', 'Lovers', 'Coworkers'],
        genre: 'What If'
    },
    {
        id: 'w6',
        text_en: "If your favorite celebrity confessed her love for you, would you dump me to date her?",
        text_ar: "لو الكراش المشهورة بتاعتك قالتلك بحبك، هتسيبني وتروحلها؟",
        category: ['Lovers'],
        genre: 'What If'
    },
    {
        id: 'w7',
        text_en: "If you could stop my face and body from aging, would you— or would you rather watch me grow old with you?",
        text_ar: "لو بإيدك تخليني مأعجزش أبداً.. هتوافق ولا تحب نشوف بعض واحنا بنعجز سوا؟",
        category: ['Lovers'],
        genre: 'What If'
    },
    {
        id: 'w8',
        text_en: "If you develop superpowers and were sworn to keep them a secret to keep me safe, would you tell me anyways?",
        text_ar: "لو بقى عندك قوى خارقة ولازم تخبيها عشان تحميني، هتقولي برضو ولا لا؟",
        category: ['Lovers'],
        genre: 'What If'
    },
    {
        id: 'w9',
        text_en: "If you had the power to read my mind, would you use it without my permission?",
        text_ar: "لو بقيت تقدر تقرأ أفكاري، هتعمل كده من غير إذني؟",
        category: ['Lovers', 'Friends'],
        genre: 'What If'
    },
    {
        id: 'w10',
        text_en: "If you could go to jail for something that I did so that I would be able to keep living my life, would you?",
        text_ar: "ممكن تدخل السجن بدالي عشان أنا أعيش حياتي وتنقذني؟",
        category: ['Lovers', 'Family'],
        genre: 'What If'
    },

    // --- Know me ---
    {
        id: 'k1',
        text_en: "What’s my go-to comfort food when I’m stressed?",
        text_ar: "إيه الأكلة اللي باكلها لما بكون مضغوط؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Know me'
    },
    {
        id: 'k2',
        text_en: "What’s something I always say or do that makes me… me?",
        text_ar: "إيه الحاجة اللي دايماً بقولها أو بعملها وبتميزني؟",
        category: ['Friends', 'Family', 'Lovers'],
        genre: 'Know me'
    },
    {
        id: 'k3',
        text_en: "What’s one thing I’d never leave the house without?",
        text_ar: "إيه الحاجة اللي مستحيل أنزل من البيت من غيرها؟",
        category: ['Friends', 'Lovers'],
        genre: 'Know me'
    },
    {
        id: 'k4',
        text_en: "If I suddenly got famous, what would it be for?",
        text_ar: "لو بقيت مشهور فجأة، تفتكر هيكون بسبب إيه؟",
        category: ['Friends', 'Family', 'Lovers'],
        genre: 'Know me'
    },
    {
        id: 'k5',
        text_en: "What’s a movie or show I probably love — even if I won’t admit it?",
        text_ar: "إيه الفيلم أو المسلسل اللي أنا بحبه بس مكسوف أعترف؟",
        category: ['Lovers', 'Friends'],
        genre: 'Know me'
    },
    {
        id: 'k6',
        text_en: "If we switched lives for a day, what do you think I'd struggle with most?",
        text_ar: "لو بدلنا حياتنا ليوم، إيه أكتر حاجة هتعب فيها في حياتك؟",
        category: ['Lovers', 'Friends'],
        genre: 'Know me'
    },
    {
        id: 'k7',
        text_en: "If you saw a stranger flirting with me, how would you react?",
        text_ar: "لو شفت حد غريب بيعاكسني، هتعمل إيه؟",
        category: ['Lovers'],
        genre: 'Know me'
    },
    {
        id: 'k8',
        text_en: "If you lost me in a supermarket, where's the first place you'd look?",
        text_ar: "لو تهت مني في السوبر ماركت، أول مكان هتدور عليا فيه فين؟",
        category: ['Lovers', 'Friends', 'Family'],
        genre: 'Know me'
    },
    {
        id: 'k9',
        text_en: "What's the most attractive thing I've ever done by accident?",
        text_ar: "إيه أكتر حركة جذابة عملتها من غير ما أقصد؟",
        category: ['Lovers'],
        genre: 'Know me'
    },
    {
        id: 'k10',
        text_en: "What's the first thing you noticed about my body language?",
        text_ar: "إيه أول حاجة لفتت نظرك في لغة الجسد بتاعتي؟",
        category: ['Lovers', 'First Date'],
        genre: 'Know me'
    },
    {
        id: 'k11',
        text_en: "What was the first thing you noticed about me when we met in person?",
        text_ar: "إيه أول حاجة لاحظتها فيا أول ما اتقابلنا؟",
        category: ['Lovers', 'First Date', 'Friends'],
        genre: 'Know me'
    },
    {
        id: 'k12',
        text_en: "What do your friends know about me, if anything?",
        text_ar: "صحابك يعرفوا إيه عني؟",
        category: ['Lovers', 'First Date'],
        genre: 'Know me'
    },
    {
        id: 'k13',
        text_en: "Any non-negotiable in your life?",
        text_ar: "إيه الحاجة اللي عندك \"خط أحمر\" في حياتك؟",
        category: ['Lovers', 'First Date', 'Friends'],
        genre: 'Know me'
    },
    {
        id: 'k14',
        text_en: "If every human came with a warning label, what would yours say?",
        text_ar: "لو كل بني آدم عليه تيكت تحذير، التيكت بتاعك مكتوب عليه إيه؟",
        category: ['Lovers', 'First Date', 'Friends', 'Coworkers'],
        genre: 'Know me'
    }
];


export const categories = [
    { id: 'Lovers', label_en: 'Lovers', label_ar: 'حبايب' },
    { id: 'Friends', label_en: 'Friends', label_ar: 'صحاب' },
    { id: 'Family', label_en: 'Family', label_ar: 'عيلة' },
    { id: 'Coworkers', label_en: 'Coworkers', label_ar: 'زمايل شغل' },
    { id: 'First Date', label_en: 'First Date', label_ar: 'أول ديت' },
    { id: 'First Time Meeting', label_en: 'First Time Meeting', label_ar: 'أول مرة نتقابل' },
];

export const genres = [
    { id: 'Funny', label_en: 'Funny', label_ar: 'ضحك' },
    { id: 'Deep', label_en: 'Deep', label_ar: 'عميق' },
    { id: 'Awkward', label_en: 'Awkward', label_ar: 'محرج' },
    { id: 'What If', label_en: 'What If...', label_ar: 'ماذا لو...' },
    { id: 'Know me', label_en: 'Know me', label_ar: 'اعرفني أكتر' },
];
